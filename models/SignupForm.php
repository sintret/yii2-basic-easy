...
class SignupForm extends Model {
    ...
    public function sendMail($user) {
        ...
        $body .= "\n";
        $body .= "Successfully registered. we will review your membership, comeback again in a hours. \n";
        $body .= "Thanks, \n";
        $body .= Yii::$app->name;
        $body_message = $this->template($subject, $body);
        $setting = Setting::find()->where(['id' => 1])->one();
        
        return \Yii::$app->mailer->compose('registration', ['user' => $user])
                                ->setFrom([$setting->emailAdmin => \Yii::$app->name . ' robot'])
                                ->setTo($mail)
                                ->setSubject('Registration for ' . \Yii::$app->name)
                                ->send();
